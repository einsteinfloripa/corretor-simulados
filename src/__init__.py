...
def main(dados):
    ...
    df_dados_alunos, df_respostas, df_gabarito = csvs_to_dfs(
        dados_alunos_path,
        respostas_alunos_path,
        gabarito_path
    )
    df_redacoes = []   # PLACE HOLDER Não implementado

    # Corrigir Provas
    df_resultado = corrigir(df_respostas, df_gabarito,
                            df_redacoes, tipo_correcao)
    ...
    student_dataset = gerar_json_alunos(
        df_resultado, df_gabarito, tipo_correcao)
